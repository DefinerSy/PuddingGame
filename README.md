# PuddingGame

基于 Web（Vite + TypeScript + Matter.js）的 2D 物理堆叠塔防原型：吊机移动、老虎机 Roll、三格取货、合法着地区随吊机移动、双侧波次敌人。界面为柔和马卡龙休闲风；画布渐变天空、草地与果冻感方块绘制集中在 `src/game/canvasArt.ts`。

## 本地运行

```bash
npm install
npm run dev
```

浏览器打开终端里提示的本地地址即可。

## 操作

- **A / D** 或 **← / →**：移动吊机（合法绿色区域随之平移）
- **空格**：抓取或释放吊机附近的布丁积木
- **R**：老虎机 Roll（等同点击按钮）
- **1 / 2 / 3**：取出从左到右第 1～3 格（与 UI 角标一致；剩两格时仍为 1、2）
- **老虎机 Roll**：消耗费用，随机三个布丁类型
- **取出**：对某一格再付费用，积木挂到吊机绳上

绿色区域内布丁落到地面会保留；红区内落地会被销毁。

**完美合体**：同类布丁上下叠放（中心大致对齐、水平投影有足够重叠、上块底与下块顶贴合）会合并为**更高的一体块**。**防御者**：耐啃约 **2.5×**。**射手**：**攻速不变**，**射程约 2.5×**，并**多发弹道**（不同敌人优先，小扇形分散）。**生产者**：**产费间隔不变**，单次产费约 **2.5×**。可多次合并累乘。参数见 `MERGE_*`。

**基地升级**：商店旁按钮付费升级（最多 4 级，费用递增）；每级**天空与可堆高度增加**、**吊机挂钩上移**、**画布缩放略拉远**、**敌人生成位置更靠边**。见 `BASE_UPGRADE_*`。

**手机 / 触屏**：在画布上左右拖动可平移吊机；底部有 ◀ / ▶ 与「抓取」按钮（窄屏或触控设备上显示）。

## 构建

```bash
npm run build
npm run preview
```

## 数值

可在 `src/game/config.ts` 中调整费用、射速、射程高度加成、波次间隔等。

平衡思路简述：开局基础 Roll/取出较低、起步费略多，且**第一次老虎机必含至少一格射手**；每**成功从商店取出一个方块**，Roll 与单次取出费用各涨一档（有上限），**整局持续累加、不因再次 Roll 而重置**；被动与生产者产费适中。首波前有短准备时间；敌人数量每 4 波每侧 +1；敌人类型含**快跑、普通、重坦、冲刺**，高波次混合刷新，各有血量/速度/啃咬差异（见 `src/game/enemies.ts`）。

## GitHub Pages

1. 仓库 **Settings → Pages**：**Build and deployment** 的 **Source** 选 **GitHub Actions**。
2. 将包含本工作流的提交合并进 **`main`** 分支（或手动运行 **Actions → Deploy to GitHub Pages → Run workflow**）。
3. 部署完成后，游戏地址一般为：`https://<你的用户名>.github.io/PuddingGame/`（仓库名区分大小写时请与 GitHub 上实际仓库名一致）。

本地若要模拟线上路径，可执行：

```bash
VITE_BASE_PATH=/PuddingGame/ npm run build && npm run preview
```
