...
- **空格**：抓取或释放吊机附近的布丁积木
- **R**：老虎机 Roll（等同点击按钮）
- **1 / 2 / 3**：取出从左到右第 1～3 格（与 UI 角标一致；剩两格时仍为 1、2）
- **老虎机 Roll**：消耗费用，随机三个布丁类型
- **取出**：对某一格再付费用，积木挂到吊机绳上

绿色区域内布丁落到地面会保留；红区内落地会被销毁。

**完美合体**：同类布丁上下叠放（中心大致对齐、水平投影有足够重叠、上块底与下块顶贴合）会合并为**更高的一体块**；功效约为单体 **2.5×**（可多次合并累乘）。参数见 `config.ts` 中 `MERGE_*`（`MERGE_FACE_GAP_*` 控制竖直接触）。
...
平衡思路简述：开局基础 Roll/取出较低、起步费略多，且**第一次老虎机必含至少一格射手**；每**成功从商店取出一个方块**，Roll 与单次取出费用各涨一档（有上限），**整局持续累加、不因再次 Roll 而重置**；被动与生产者产费适中。首波前有短准备时间；敌人数量每 4 波每侧 +1；敌人类型含**快跑、普通、重坦、冲刺**，高波次混合刷新，各有血量/速度/啃咬差异（见 `src/game/enemies.ts`）。
